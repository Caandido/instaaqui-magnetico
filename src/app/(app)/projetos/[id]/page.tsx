...
export default async function ResultadosPage({
  ...
  if (project === null) {
    // Sem sessão → login; com sessão mas sem acesso → 404.
    const anon = await db.project.findUnique({ where: { id } });
    if (!anon) notFound();
    redirect("/login");
  }
  if (!project) notFound();
  ...
  const [ideas, scripts, gaps, trends, insights, report, lastRun, alerts] =
    await Promise.all([
      ...
      db.gap.findMany({ where: { projectId: id }, orderBy: { createdAt: "asc" } }),
      db.trend.findMany({ where: { projectId: id }, orderBy: { createdAt: "asc" } }),
      db.insight.findMany({ where: { projectId: id }, orderBy: { createdAt: "asc" } }),
      db.report.findFirst({ where: { projectId: id }, orderBy: { createdAt: "desc" } }),
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <Link href="/projetos" className="text-sm text-pink-700 hover:underline">
            ← Projetos
          </Link>
          <h1 className="mt-1 text-2xl font-semibold">{project.name}</h1>
          <p className="text-gray-600">Resultados da análise de IA</p>
        </div>
        {lastRun?.finishedAt && (
          <p className="text-sm text-gray-400">
            Última análise:{" "}
            {lastRun.finishedAt.toLocaleString("pt-BR")} · {lastRun.status}
          </p>
        )}
      </div>
      ...
      {!hasAnything ? (
        <p className="rounded-lg border border-dashed border-gray-300 bg-white p-5 text-sm text-gray-500">
          Nenhuma análise ainda. Volte aos Projetos e clique em “Analisar com IA”
          (colete os concorrentes antes).
        </p>
      ) : (
